...
def example_processor(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = client.rerank_and_sort(data['query'], pos_contents)
    data['pos'] = [{"content":item["content"], "score": (item["score"]+1)/2} for item in results]
    neg_contents = [item['content'] for item in data['neg']]
    results = client.rerank_and_sort(data['query'], neg_contents)
    data['neg'] = [{"content":item["content"], "score": (item["score"])/2} for item in results]
