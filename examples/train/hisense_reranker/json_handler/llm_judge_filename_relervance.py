...
def example_processor(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for neg in data['neg']:
        ...
        prompt = f'''请判断以下的文件名是否和指定的问题中产品类型和型号是否一致,：{query}，
，需要判断的文件名：{neg['filename']}
输出两行，第一行是1或者0，1,2 1表示完全一致，0表示不一致，2表示文件名不涉及具体型号，第二行输出reason，不要带任何格式和标题'''
        ...
        neg['llm_filename_relervance'] ,neg['llm_filename_reason'] = response['choices'][0]['message']['content'].split('\n')
