...
class RerankClient:
    def __init__(self, base_url: str = "http://10.18.231.62:31114/"):
        """初始化Rerank客户端"""
        self.base_url = base_url
        self.rerank_endpoint = f"{self.base_url}/encode_corpus"
    ...
    def rerank_batch(self, query: str, documents: List[str], instruction: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            #import pdb;pdb.set_trace()
            vectors = [item['value'] for item in result['data']]
            query_vector = vectors[0]
            scores=[]
            for vector in vectors[1:]:
                score = self.calculate_dot_product(query_vector, vector)
                scores.append(score)
            logging.debug(f"批次排序请求成功，返回{result}")
    ...
    def do_rerank(self, query: str, documents: List[str], instruction: Optional[str] = None) -> List[Dict]:
        ...
        scores_x = [item["score"] for item in ranked_docs]
        print(f'##{scores_x}')
        return ranked_docs
...
def example_processor(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    #client.rerank_batch(data['query'],data['segments'])
    results = client.do_rerank(data.get('query', ''), data['segments'])
    #import pdb;pdb.set_trace()
    #print(results)
    new_data={}
    new_data['query'] = data['query']
    new_data['segments'] = [item['content'] for item in results]
