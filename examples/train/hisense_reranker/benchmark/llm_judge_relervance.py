...
def example_processor(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for item in data['value'][:3]:
        ...
        if 'qna_title' in item.keys():
            qna_title = item['qna_title']
            qna_content = item['qna_content']
            candidates.append({'block':f'{qna_title}\n{qna_content}' ,'filename':item['filename'],'qna':True})
        else:
            lack_block = False
            if 'block' in item.keys():
                ...
                lack_block = True
                block = item['document']
            candidates.append({'block':block ,'filename':item['filename'],'qna':True})
        #print(item)
